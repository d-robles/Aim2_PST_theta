...
parts = {'0003', '0004', '0007','0008', '0009', '0011', '0012', '0014', '0015'};
session = {'S02'};
tmstarget = {'10Hz'};
%conditions = {'positive';'negative'};
filepath = 'D:\PST_2a\raw_files\';% C:\Users\Kathryn\Documents\PST_DR\
exp = 'PST';
...
for i_part = 1:length(parts)
    ...
    for i_sess = 1:length(session)
        for i_targ = 1:length (tmstarget)
            ...
            filename = [exp session{i_sess} '_' tmstarget{i_targ} '_' parts{i_part} '.vhdr'];
            EEG = pop_loadbv(filepath, filename, [], []); % loading in EEG data
